...
export function getMeasVal(meas: MeasEnt): number {
  ...
  if (message.hasOwnProperty(valueTag)) {
    ...
    console.error(`[ERROR] Measurement did not have sought tag value ${valueTag}`)
  }
}
...
export function calcSpd(
  ...
  const spdBeforeMPS = getMeasVal(spdBefore) / 3.6; // why 3.6?

  const dateAfter = spdAfter.Created_Date.getTime();
  const spdAfterMPS = getMeasVal(spdAfter) / 3.6; // why 3.6?
