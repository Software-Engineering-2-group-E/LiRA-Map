...
const RideGraphCard: React.FC<{ paths: MeasMetaPath; selectedMeasurements: ActiveMeasProperties[]; }> = ({
																											 paths,
		...
		const datasets: ChartDataset<'line', Object[]>[] = selectedMeasurements
			...
			.flatMap(({ name }) => {
				return Object.values(Object.values(paths[name])[0]).flatMap((trip: BoundedPath) => {
					const data = trip.path.map((o: any) => ({
						x: Date.parse(o.metadata.timestamp),
						y: o.value ? o.value : 0,
						lat: o.lat,
						lng: o.lng,
					}));
		...
		<>
			{datasets.length > 0 && (
				<Card sx={{ position: 'absolute', bottom: '20px', right: '10px', zIndex: 1000 }}>
					<CardContent>
						<IconButton onClick={() => setOpen(!open)}>{open ? <ExpandMoreIcon /> :
